...
st.markdown('<div class="main-title">🌊 OceanEmbed Framework</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Satellite Embedding-Based Deep Learning for 3D Subsurface Ocean Temperature Reconstruction</div>', unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.header("📍 Location Inspector")
...
if metrics:
    ...
    with col1:
        st.metric("Overall 15-Depth RMSE", f"{metrics.get('rmse_degC', 0.0):.3f} °C")
    with col2:
        st.metric("Pearson Correlation", f"{metrics.get('correlation', 0.0):.4f}")
    with col3:
        st.metric("Model Bias", f"{metrics.get('bias_degC', 0.0):.3f} °C")
    with col4:
        st.metric("3D Target Grid", f"15 Depths × {len(lats)} × {len(lons)}")

st.markdown("---")

# Main Content Tabs
tab1, tab2, tab3, tab4 = st.tabs(["🗺️ 2D Temperature Field", "🧊 3D Ocean Volume", "📈 Vertical Profile", "📊 Diagnostics"])
...
with tab1:
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f"({selected_lat}°N, {selected_lon}°E)"],
...
with tab2:
    st.subheader(f"3D Subsurface Ocean Temperature Volume Map ({dates[selected_date_idx]})")
    ...
    if not selected_3d_depths:
        ...
        # Render 3D surface plane for each selected depth
        for d_val in sorted(selected_3d_depths):
            ...
            # Use visual Z index for clear vertical separation
...
with tab3:
    st.subheader(f"Vertical Subsurface Temperature Profile at ({selected_lat}°N, {selected_lon}°E)")
    
    lat_idx = np.abs(lats - selected_lat).argmin()
    lon_idx = np.abs(lons - selected_lon).argmin()

    is_ocean = ocean_mask[lat_idx, lon_idx]
    
    if not is_ocean:
        st.warning(f"Selected location ({lats[lat_idx]:.2f}°N, {lons[lon_idx]:.2f}°E) is over LAND. Please select an ocean grid cell.")
    else:
        ...
        fig_prof.add_trace(go.Scatter(
            ...
            y=depths,
            mode='lines+markers',
            name='OceanEmbed 15-Depth Prediction',
...
with tab4:
    st.subheader("Model Performance & Depth-Wise Diagnostics")
    
    col_a, col_b = st.columns(2)
    with col_a:
        if os.path.exists("reports/loss_curve.png"):
            st.image("reports/loss_curve.png", caption="Training & Validation Loss Curve", use_container_width=True)
        else:
            st.info("Loss curve plot not found.")
            
    with col_b:
        if os.path.exists("reports/rmse_by_depth.png"):
            st.image("reports/rmse_by_depth.png", caption="RMSE by Depth Level (0m down to 1000m)", use_container_width=True)
        else:
            st.info("RMSE by depth plot not found.")

st.markdown("---")
st.caption("OceanEmbed Project Prototype - SIH 2026 | Built with PyTorch, Xarray, Streamlit & Plotly")
